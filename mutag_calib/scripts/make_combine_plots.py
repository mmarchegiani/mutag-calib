...
def plot_one(f, group, channel, out_png, ratio_ylim=(0.4, 1.6)):
    d = f[f"{group}"][channel]

    b, edges, _ = fetch_hist(d, "b_2022_preEE")
    c, _, _     = fetch_hist(d, "c_2022_preEE")
    l, _, _     = fetch_hist(d, "light_2022_preEE")
...
def main():
    ...
    with uproot.open(args.file) as f:
        plot_one(f, "shapes_prefit", args.passch,
                 os.path.join(args.outdir, "prefit_pass_bcl_data_band.pdf"), (rmin, rmax))
        plot_one(f, "shapes_prefit", args.failch,
                 os.path.join(args.outdir, "prefit_fail_bcl_data_band.pdf"), (rmin, rmax))
        plot_one(f, args.postfit_group, args.passch,
                 os.path.join(args.outdir, "postfit_sb_pass_bcl_data_band.pdf"), (rmin, rmax))
        plot_one(f, args.postfit_group, args.failch,
                 os.path.join(args.outdir, "postfit_sb_fail_bcl_data_band.pdf"), (rmin, rmax))
